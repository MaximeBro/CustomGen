...
public class GenCommand implements CommandExecutor {
        ...
        if(sender instanceof Player player) {
            ...
            if(args[0].equalsIgnoreCase("give") && args.length > 1) {
                ...
                if(this.plugin.dataLoader.contains(name)) {
                    ...
                    player.sendMessage("§b§lCustomGen §8- §ayou were given §7" + nb + " §b§n" + name + "§r §agenerator(s) !");
                    return true;
                } else {
            ...
            if( args[0].equalsIgnoreCase("gen") ||
                ...
                args[0].equalsIgnoreCase("generators")) {
                ...
                for(GenGeneration generation : this.plugin.dataLoader.getGenerations()) {
                    genList += " §8- §3" + generation.getName() + "§r\n";
                }
            ...
            if(args[0].equalsIgnoreCase("generation") && args.length > 1) {
                String name = args[1];
                if(this.plugin.dataLoader.contains(name)) {
                    String generation = "Generation of " + name + "\n";
                    for(Map.Entry<Material, Double> entry : this.plugin.dataLoader.getGeneration(name).getGeneration().entrySet()) {
                        generation += entry.getKey().name() + ": " + entry.getValue() + "%" + "\n";
                    }
    ...
    private String getHelpMessage() {
        ...
        message += "\n" + "§3/customgen";
        message += "\n" + "§3/customgen help";
        message += "\n" + "§3/customgen list | gen | generators";
